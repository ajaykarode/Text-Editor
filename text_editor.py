...
color_dict = {
    'Light Default' : ('#000000', '#ffffff'),         #here in bracket first number is for text color,and second number
    'Light Plus' : ('#474747', '#e0e0e0'),            #that is after comma(,) is for background color.
    'Dark' : ('#c4c4c4', '#2d2d2d'),     
    'Red' : ('#2d2d2d', '#ffe8e8'),
...
#FONT FAMILY ANS FONT SIZE FUNCTIONALITY
...
#3)underline button functionality
...
#4)font color button functionality
...
def changed(event=None):
    ...
    if text_editor.edit_modified():
        text_changed = True
        words = len(text_editor.get(1.0,'end-1c').split())   #if we did not want to count spaces in characters then we can 
        characters = len(text_editor.get(1.0,'end-1c'))      #add [.replace('' ,'')]  in line 3857 at last 
        status_bar.config(text=f'Total Characters : {characters}    Total Words : {words}')
...
file.add_command(label='SaveAs',image=save_as_icon,compound=tk.LEFT,accelerator='ctrl+Shift+s', command=save_as)
